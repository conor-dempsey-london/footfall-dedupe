...
class LinearPoolB1(ModelBuilder):
    # Give the model a name
    _model_type = "LinearPoolB1"
...
class LinearPartPoolB1(ModelBuilder):
    # Give the model a name
    _model_type = "LinearPartPoolB1"
...
class LinearPoolB1PoolB2(ModelBuilder):
    # Give the model a name
    _model_type = "LinearPoolB1PoolB2"
    ...
    def build_model(self, X: pd.DataFrame, y: pd.Series, **kwargs):
        ...
        with pm.Model(coords=coords) as self.model:
            ...
            # prior on the slope
            mu_β1 = self.model_config.get("β1_mu_prior", 0.005)
            sigma_β1 = self.model_config.get("β1_sigma_prior", 0.01)
            β1 = pm.Gamma("β1", mu=mu_β1, sigma=sigma_β1)

            # prior on the interaction coefficient
            mu_β2 = self.model_config.get("β2_mu_prior", 0.005)
            sigma_β2 = self.model_config.get("β2_sigma_prior", 0.01)
            β2 = pm.Gamma("β2", mu=mu_β2, sigma=sigma_β2)

            # model error
            σ = pm.HalfCauchy("σ", σ_prior)

            # linear mean 
            mu = β1*x_data + β2*x_data*m_data
...
class LinearPartPoolB2(ModelBuilder):
    # Give the model a name
    _model_type = "LinearPartPoolB2"
    ...
    def build_model(self, X: pd.DataFrame, y: pd.Series, **kwargs):
        ...
        with pm.Model(coords=coords) as self.model:
            ...
            sigma_β2 = pm.HalfNormal("sigma_β2", sigma=β2_sigma_prior)

            # group-specific slope priors
            β2 = pm.Gamma("β2", mu=mu_β2, sigma=sigma_β2, dims="area")

            # model error
            σ = pm.HalfCauchy("σ", σ_prior)

            # linear mean 
            mu = β2[area_idx]*m_data*x_data
...
class LinearPartPoolB2PoolB1(ModelBuilder):
    # Give the model a name
    _model_type = "LinearPartPoolB2PoolB1"
    ...
    def build_model(self, X: pd.DataFrame, y: pd.Series, **kwargs):
        ...
        with pm.Model(coords=coords) as self.model:
            ...
            # linear mean 
            mu = β1*x_data + β2[area_idx]*m_data*x_data
